...
class ComplianceFramework(Base):
    """Compliance framework definition (e.g., NIST 800-53, ISO 27001, CIS v8)."""
    __tablename__ = "compliance_frameworks"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False)
    key = Column(String(50), nullable=False, index=True)        # e.g. "nist-800-53", "iso-27001", "cis-v8"
    name = Column(String(255), nullable=False)                   # e.g. "NIST SP 800-53 Rev 5"
    version = Column(String(50), nullable=True)                  # e.g. "Rev 5", "2022"
    description = Column(Text, nullable=True)
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
...
class ComplianceControl(Base):
    ...
    __table_args__ = (
        UniqueConstraint("framework_id", "control_id", name="uq_framework_control_id"),
    )

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False)
    framework_id = Column(UUID(as_uuid=True), ForeignKey("compliance_frameworks.id"), nullable=False)
    control_id = Column(String(50), nullable=False, index=True)  # e.g. "AC-1", "A.5.1.1", "1.1"
    title = Column(String(500), nullable=False)
    description = Column(Text, nullable=True)
    family = Column(String(100), nullable=True)                   # e.g. "Access Control", "Annex A.5"
    priority = Column(String(20), nullable=True)                  # P1, P2, P3 (NIST) or similar
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
...
class ComplianceMapping(Base):
    """Maps threats or recommendations to compliance controls."""
    __tablename__ = "compliance_mappings"
    __table_args__ = (
        UniqueConstraint("control_id", "threat_id", name="uq_control_threat",),
    )
    ...
    threat_id = Column(UUID(as_uuid=True), ForeignKey("threats.id"), nullable=True)
    assessment_id = Column(UUID(as_uuid=True), ForeignKey("assessments.id"), nullable=True)
    status = Column(String(30), default="not_assessed")           # not_assessed, compliant, non_compliant, partially_compliant, not_applicable
    notes = Column(Text, nullable=True)
    evidence_ids = Column(JSONB, default=[])                      # list of evidence UUIDs supporting this mapping
    mapped_by = Column(String(20), default="manual")              # manual, ai_suggested
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow)
