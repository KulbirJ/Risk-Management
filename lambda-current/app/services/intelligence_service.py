...
Focus on architectural risks, known CVEs for the tech stack, and scope-related security gaps."""
    ...
    def _extract_nessus_findings(
        ...
        if xml_start == -1:
            logger.info(
                "Nessus file %s: raw XML not found, falling back to Bedrock",
                evidence.file_name,
            )
        ...
        logger.info(
            "Nessus parser: %d findings from %s (%d unique plugins, %d with CVEs)",
            len(findings), evidence.file_name, len(seen_plugins),
